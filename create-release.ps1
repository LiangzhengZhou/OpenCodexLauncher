...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).FileVersion -ne '2.6.5.0') { throw 'Executable version does not match release.' }
...
[IO.Compression.ZipFile]::CreateFromDirectory($sourceDir,(Join-Path $releaseRoot 'OpenCodexLauncher-2.6.5-source.zip'))
[IO.Compression.ZipFile]::CreateFromDirectory($binaryDir,(Join-Path $releaseRoot 'OpenCodexLauncher-2.6.5-windows-x64.zip'))
