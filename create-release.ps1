...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).FileVersion -ne '3.0.3.0') { throw 'Executable version does not match 3.0.3 stable.' }
...
[IO.Compression.ZipFile]::CreateFromDirectory($sourceDir,(Join-Path $releaseRoot 'OpenCodexLauncher-3.0.3-source.zip'))
[IO.Compression.ZipFile]::CreateFromDirectory($binaryDir,(Join-Path $releaseRoot 'OpenCodexLauncher-3.0.3-windows-x64.zip'))
