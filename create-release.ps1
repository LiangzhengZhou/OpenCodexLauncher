...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).FileVersion -ne '2.5.1.0') { throw 'Executable version does not match release.' }
...
[IO.Compression.ZipFile]::CreateFromDirectory($sourceDir,(Join-Path $releaseRoot 'OpenCodexLauncher-2.5.1-source.zip'))
[IO.Compression.ZipFile]::CreateFromDirectory($binaryDir,(Join-Path $releaseRoot 'OpenCodexLauncher-2.5.1-windows-x64.zip'))
