...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).FileVersion -ne '3.0.0.0') { throw 'Executable version does not match 3.0 preview.' }
...
[IO.Compression.ZipFile]::CreateFromDirectory($sourceDir,(Join-Path $releaseRoot 'OpenCodexLauncher-3.0.0-preview-source.zip'))
[IO.Compression.ZipFile]::CreateFromDirectory($binaryDir,(Join-Path $releaseRoot 'OpenCodexLauncher-3.0.0-preview-windows-x64.zip'))
