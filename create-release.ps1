...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).FileVersion -ne '2.5.2.0') { throw 'Executable version does not match release.' }
...
[IO.Compression.ZipFile]::CreateFromDirectory($sourceDir,(Join-Path $releaseRoot 'OpenCodexLauncher-2.5.2-source.zip'))
[IO.Compression.ZipFile]::CreateFromDirectory($binaryDir,(Join-Path $releaseRoot 'OpenCodexLauncher-2.5.2-windows-x64.zip'))
