...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).FileVersion -ne '2.6.3.0') { throw 'Executable version does not match release.' }
...
[IO.Compression.ZipFile]::CreateFromDirectory($sourceDir,(Join-Path $releaseRoot 'OpenCodexLauncher-2.6.3-source.zip'))
[IO.Compression.ZipFile]::CreateFromDirectory($binaryDir,(Join-Path $releaseRoot 'OpenCodexLauncher-2.6.3-windows-x64.zip'))
