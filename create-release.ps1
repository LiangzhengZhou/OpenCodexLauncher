...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).FileVersion -ne '3.1.1.0') { throw 'Executable version does not match 3.1.1 stable.' }
...
[IO.Compression.ZipFile]::CreateFromDirectory($sourceDir,(Join-Path $releaseRoot 'OpenCodexLauncher-3.1.1-source.zip'))
[IO.Compression.ZipFile]::CreateFromDirectory($binaryDir,(Join-Path $releaseRoot 'OpenCodexLauncher-3.1.1-windows-x64.zip'))
