...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($exe).FileVersion -ne '3.1.2.0') { throw 'Executable version does not match 3.1.2 stable.' }
...
[IO.Compression.ZipFile]::CreateFromDirectory($sourceDir,(Join-Path $releaseRoot 'OpenCodexLauncher-3.1.2-source.zip'))
[IO.Compression.ZipFile]::CreateFromDirectory($binaryDir,(Join-Path $releaseRoot 'OpenCodexLauncher-3.1.2-windows-x64.zip'))
