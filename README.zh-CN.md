...
启动后，Windows 通知区域会出现粉色图标。点击关闭（×）、按 Alt+F4 或最小化时，主窗口隐藏，后台操作继续。左键点击托盘图标，或右键选择 **显示主窗口**，可恢复原窗口和编辑内容。右键选择 **退出启动器** 才会完全退出并取消启动器操作；不会主动停止单独运行的 OpenCodex 代理或 Desktop。

图标可能放在任务栏 **^** 展开的隐藏图标区，可自行拖到通知区域常显。本功能不启用 Windows 开机自启动。软件内更新和回退仍会真正退出后替换文件；手动替换文件前请使用 **退出启动器**，只点 × 不会退出进程。托盘初始化失败时会提示，本次关闭窗口将正常退出。

## 选择版本并一键回退

1. 保存表单修改，关闭其他启动器窗口，打开 **设置 → 版本管理**。
2. 在关键版本列表中选择 **2.6.5**，点击 **一键回退到所选版本**。
3. 核对目标版本及兼容检查结果并确认。软件校验下载包、备份程序文件后，自动关闭启动器并打开所选版本。

列表仅收录发布时明确确认的关键版本，不自动纳入所有 GitHub Release。保留配置格式 v1、供应商、API Key、模型选择、Desktop 关联和运行时路径；隔离测试覆盖真实 2.6.5 的配置读取与 DPAPI 凭据解密。不会用历史快照覆盖当前配置，也不会重启 Desktop 或 OpenCodex。2.6.5 使用旧界面，没有版本选择列表；可通过“检查启动器更新”再次升级到最新正式版。

下载包大小和 SHA-256 已固定，同时检查包内文件与 EXE 版本。成功下载并校验后保留本机缓存，之后可离线使用；缓存损坏会停止回退，请先移走本机 updates/critical-cache 下的损坏包再重试。启用 Reserve Force 或配置不兼容时停止操作。程序文件备份保留在私有 recovery 目录，写入或启动命令失败时尝试恢复；发现外部修改则停止覆盖并保留恢复记录。进程启动成功不等于已经验证全部功能。

此列表仅切换 **启动器**，OpenCodex 运行时的关键回退版本尚未确认。
...
只有完成首次配置并明确点击才开始检查。不改配置、不导入凭据、不运行 CLI、不停止服务、不重启 Desktop。网络检查向数字回环地址候选端口发送 GET /healthz，并对目标回环端口进行一次 TCP 连接及标准消息地址 GET，不带凭据、不走系统代理、不记录响应体、不跟随重定向。每个文件最多读取 2 MiB，只检查本地固定磁盘；链接、网络路径、相对目录引用或不支持的 TOML 语法保留为未验证。总计等待上限 12 秒，从托盘菜单退出启动器会取消检查；若系统调用仍阻塞，则返回超时报告并忽略迟到结果。
...
更新助手等待当前启动器正常退出后，备份并替换软件文件，再打开新版。不会停止 OpenCodex 服务，也不改动设置、凭据、运行时路径和无关文件。请先关闭其他启动器窗口。写入失败会尝试回滚；发现外部修改则停止覆盖并保留恢复资料。安装目录必须可写，不会自动申请管理员权限。取消下载不改动安装文件；不会后台检查。普通更新仅接受更高版本，降级必须使用独立的关键版本回退入口。

2.6.0 之前的版本需要最后手动下载、完整解压一次新版 Windows ZIP，获得此功能。之后就可以使用软件内按钮更新启动器；下方的 OpenCodex 更新仍是独立功能。

若更新中断电，请保留 %LOCALAPPDATA%/OpenCodexLauncher/updates 和 recovery。updates/<id>/recovery.txt 指向备份目录，journal.json 记录安装文件与编号 .original 备份的对应关系。从托盘菜单退出全部启动器实例，比较文件后恢复完整的旧文件集；没有 BeforeFile 的条目表示更新中新建的文件。保留本地应用数据和无关文件，不要直接覆盖其他程序的改动。也可以完整解压已校验的 Windows ZIP 恢复。中断事务不会自动恢复；旧备份由用户按需清理，这些私有目录不可分享。
...
需要能访问 nodejs.org 和 registry.npmjs.org 的网络，可能耗时数分钟；下载包不含离线运行环境。每次安装在 %LOCALAPPDATA%/OpenCodexLauncher/runtimes 下新建独立目录，验证成功后才保存所选路径。失败或取消保留原路径与配置，取消或从托盘菜单退出启动器会终止安装器启动的进程组，隐藏窗口不会终止安装。失败暂存和历史版本保留用于诊断、回退，每份可能占用数百 MB；仅在确认无进程、服务或保存路径使用时手动清理，保留当前和上一版本。此目录属于私有数据，不能上传。
