...
只有完成首次配置并明确点击才开始检查。不改配置、不导入凭据、不运行 CLI、不停止服务、不重启 Desktop。网络检查只向数字回环地址的候选端口发送 GET /healthz，不带凭据、不走系统代理、不记录响应体、不跟随重定向。每个文件最多读取 2 MiB，只检查本地固定磁盘；链接、网络路径、相对目录引用或不支持的 TOML 语法保留为未验证。总计等待上限 12 秒，关闭启动器会取消检查；若系统调用仍阻塞，则返回超时报告并忽略迟到结果。

报告仅含固定问题码、数量、目录关系、时间戳及启动器版本，不含原始配置、程序/请求日志、命令行、密钥、账号数据、供应商/模型名称、URL 或绝对路径。复制和保存均由用户主动操作，不会自动上传。报告用于选择后续修复措施，不代表远端模型可用或 Desktop 已加载模型。
